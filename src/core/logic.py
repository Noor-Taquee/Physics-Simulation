...
class Body:
  ...
  def __init__(
    ...
    self.eventMap: BodyEventMap = {"update": [], "reset": []}

  def register(self):
    """Adds the body in the calculation loop"""
    if simState.bodies.__contains__(self):
      return
  ...
  def remove_event_listener(
    self, name: BodyEventType, callback: Callable[["Body"], Any]
  ):
...
def mainloop(time: float):
  ...
  for i in range(len(simState.bodies)):
    ...
    for j in range(len(simState.bodies)):
      ...
      if dis == 0 or dis <= simState.min_dis_for_calc:
        continue
